...
export default function SessionHistoryTable({ items, onRelaunch, relaunchingId = null }) {
  return (
        ...
        <div key={item.id} className="table-row table-grid-history">
          ...
          <div className="table-actions">
            <ActionButton
              tone="default"
              size="sm"
              onClick={() => onRelaunch?.(item.id)}
              disabled={relaunchingId === item.id}
            >
              {relaunchingId === item.id ? "Повтор..." : "Повторить"}
            </ActionButton>
          </div>
        </div>
      ))}
